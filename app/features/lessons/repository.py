...
class LessonRepository:
    ...
    async def get_by_module_id(self, module_id: int, skip: int = 0, limit: int = 100) -> List[Lesson]:
    ...
    async def get_by_course_id(self, course_id: int, skip: int = 0, limit: int = 100) -> List[Lesson]:
    ...
    async def get_all(self, skip: int = 0, limit: int = 100) -> List[Lesson]:
        ...
        result = await self.session.execute(
            select(Lesson).order_by(Lesson.course_id, Lesson.module_id, Lesson.order).offset(skip).limit(limit)
        )
        return list(result.scalars().all())
...
class UserLessonRepository:
    ...
    async def get_by_user_and_lesson(self, user_id: int, lesson_id: int) -> Optional[UserLesson]:
    ...
    async def get_by_user_and_module(self, user_id: int, module_id: int) -> List[UserLesson]:
    ...
    async def get_by_user_and_course(self, user_id: int, course_id: int) -> List[UserLesson]:
