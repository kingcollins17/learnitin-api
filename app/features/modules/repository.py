...
class ModuleRepository:
    ...
    async def get_previous_module(
        self, course_id: int, current_order: int
    ) -> Optional[Module]:
        """Get the module immediately before the current one in the same course."""
        result = await self.session.execute(
            select(Module)
            .where(Module.course_id == course_id)
            .where(Module.order < current_order)
            .order_by(desc(Module.order))
            .limit(1)
        )
        return result.scalar_one_or_none()
    ...
    async def get_all(self, skip: int = 0, limit: int = 100) -> List[Module]:
        ...
        result = await self.session.execute(
            select(Module)
            .order_by(Module.course_id, Module.order)
            .offset(skip)
            .limit(limit)
        )
        return list(result.scalars().all())
