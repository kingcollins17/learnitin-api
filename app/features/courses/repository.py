...
class CourseRepository:
    ...
    async def get_by_user_id(self, user_id: int, skip: int = 0, limit: int = 100) -> List[Course]:
...
class UserCourseRepository:
    ...
    async def get_by_user_and_course(self, user_id: int, course_id: int) -> Optional[UserCourse]:
    ...
    async def get_by_user(self, user_id: int, skip: int = 0, limit: int = 100) -> List[UserCourse]:
        """Get all courses for a specific user."""
        result = await self.session.execute(
            select(UserCourse)
            .where(UserCourse.user_id == user_id)
